...
public:
   ...
   string derive_wif_key(string brain_key, int index, const string& symbol="JWOCC")
   {
   ...
   full_transaction invoke_contract(const string& caller_account_name, const string& gas_price, const string& gas_limit, const string& contract_address_or_name, const string& contract_api, const string& contract_arg)
   {
	   try {
		   ...
		   //FC_ASSERT(contract_invoke_op.gas_price >= 1000);
   ...
   full_transaction set_miner_pledge_pay_back_rate(const string& miner, int pledge_pay_back_rate,bool broadcast)
   {
	   try {
		   ...
		   FC_ASSERT(!is_locked());
		   FC_ASSERT(pledge_pay_back_rate >= 1 && pledge_pay_back_rate <= 15, "pledge_pay_back_rate must between 0 20");
   ...
   full_transaction guard_appointed_crosschain_fee(const string& account, const share_type fee, const string& symbol, int64_t expiration_time, bool broadcast)
   {
	   try {
		   FC_ASSERT(!is_locked());
   ...
   full_transaction guard_appointed_lockbalance_guard(const string& account, const std::map<string, asset>& lockbalance, int64_t expiration_time, bool broadcast)
   {
	   try {
		   FC_ASSERT(!is_locked());
   ...
   full_transaction guard_determine_block_payment(const string& account, const std::map<uint32_t, uint32_t>& blocks_pays, int64_t expiration_time, bool broadcast)
   {
	   try {
		   FC_ASSERT(!is_locked());
		   guard_determine_block_payment_operation op;
		   auto guard_member_account = get_guard_member(account);
		   const chain_parameters& current_params = get_global_properties().parameters;
   ...
   full_transaction proposal_block_address(const string& account, const fc::flat_set<address>& block_addr, int64_t expiration_time, bool broadcast)
   {
	   try {
		   ...
		   prop_op.fee_paying_account = get_account(account).addr;
		   prop_op.proposed_ops.emplace_back(op);
		   prop_op.type = vote_id_type::guard;
		   current_params.current_fees->set_fee(prop_op.proposed_ops.back().op);
		   tx.operations.push_back(prop_op);
		   set_operation_fees(tx, current_params.current_fees);
		   tx.validate();
   ...
   full_transaction guard_determine_withdraw_deposit(const string& account, bool can,const string& symbol, int64_t expiration_time, bool broadcast)
   {
	   try {
		   FC_ASSERT(!is_locked());
		   guard_determine_withdraw_deposit_operation op;
		   auto guard_member_account = get_guard_member(account);
		   const chain_parameters& current_params = get_global_properties().parameters;
		   op.can = can;
		   op.symbol = symbol;
                                  ...
                                  string memo, bool broadcast = false)
   {
	  ...
	  issue_op.publisher_addr = issuer.addr;
      if( memo.size() )
      {
...
{
	return my->wallet_create_asset(issuer, symbol, precision,max_supply, broadcast);
}
...
full_transaction wallet_api::create_guard_member(string account, bool broadcast /* = false */)
{
   return my->create_guard_member( account, broadcast);
}


full_transaction wallet_api::update_guard_formal(string proposing_account, map<account_id_type, account_id_type> replace_queue,int64_t expiration_time,
	bool broadcast /* = false */)
{
...
full_transaction wallet_api::guard_appointed_crosschain_fee(const string& account, const share_type fee, const string& symbol, int64_t expiration_time, bool broadcast)
{
	return my->guard_appointed_crosschain_fee(account,fee,symbol, expiration_time,broadcast);
}

full_transaction wallet_api::guard_appointed_lockbalance_guard(const string& account, const std::map<string, asset>& lockbalance, int64_t expiration_time, bool broadcast)
{
	return my->guard_appointed_lockbalance_guard(account, lockbalance, expiration_time, broadcast);
}
full_transaction wallet_api::guard_determine_block_payment(const string& account, const std::map<uint32_t, uint32_t>& blocks_pays, int64_t expiration_time, bool broadcast)
{
	return my->guard_determine_block_payment(account, blocks_pays,expiration_time,broadcast);
}

full_transaction wallet_api::guard_determine_withdraw_deposit(const string& account, bool can,const string& symbol, int64_t expiration_time, bool broadcast)
{
	return my->guard_determine_withdraw_deposit(account, can, symbol,expiration_time, broadcast);
}
...
void wallet_api::start_mining(const vector<string>& accts)
{
	...
	for (auto acct : accts)
	{
		...
		FC_ASSERT(witness.valid(),"only miner can mine");
